...
def partOne():
    ...
    board = []
    pooter = Pooter(data)
    spring_script = "NOT A T-NOT B J-OR T J-NOT C T-OR T J-AND D J-NOT D T-OR E T-OR H T-AND T J"
    go = "RUN"
    i = 0
            ...
            if delta > 0.5:
                ...
                while True:
                    ...
                    if result == "input":
                        commands = []
                        if i == 0:
                            commands = getCommandASCII(spring_script)
                        elif i == 1:
                            commands = getCommandASCII(go)
                        for c in commands:
                            pooter.addParams(c)
                        i += 1
                        break
                        ...
                        running_game = False
                        print(f"PartTwo: Hull damage = {pooter.register}")
                        print("done")
                        break
                    elif result == "output":
...
def getData():
    input = open("day-21-input.txt").read().split(",")
    data = []
    for i in input:
        data.append(int(i))
